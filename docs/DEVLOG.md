...
```typescript
// themeStore.ts 핵심 구조
const savedTheme = (localStorage.getItem('theme') as Theme) ?? 'dark'  // 기본값 dark
...
| Frontend 테스트 | ✅ **완료** — Vitest store 단위 20개 + Playwright E2E 4 spec | CI 9단계에서 스테이징 대상 자동 실행 |
...
| ADR-09 | 통합 테스트에 Prisma 모킹 | DB 없이 HTTP 계약 검증 목적에 충분 | **61개** 테스트(auth·patients·visits·prescriptions·payments·claims·drugs·plugins), CI 실제 DB는 migrate 검증용 |
...
| **합계 (Jest/Vitest)** | **15개 파일** | **103개 (전체 PASS)** | |
| Frontend E2E | smoke.spec.ts | - | Playwright |
| Frontend E2E | api.spec.ts | - | Playwright |
| Frontend E2E | reception.spec.ts | - | Playwright |
| Frontend E2E | workflow-navigation.spec.ts | - | Playwright |
| **E2E 합계** | **4 spec 파일** | **CI 9단계에서 스테이징 URL 대상 실행** | |
