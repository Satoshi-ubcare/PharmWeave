...
> 이 섹션만 읽어도 PharmWeave의 전체 맥락을 파악할 수 있습니다.
> 각 항목의 상세 내용은 본문 해당 섹션을 참조하세요.

### 한 문장 요약

PharmWeave는 **WinForms 기반 설치형 약국 PMS를 Web으로 전환**하고, 실제 약국 업무 흐름을 그대로 반영한 **6단계 Workflow UI**와 **Plugin 확장 구조**를 제공하는 차세대 약국 관리 플랫폼이다.
...
| # | 문제 | 영향 |
|---|------|------|
| 1 | **OS 종속성** — Windows 전용 설치형, 원격 접근 불가 | PC 교체·업데이트 때마다 재설치 필요 |
| 2 | **UI-업무 불일치** — 기능 메뉴 중심 vs 실제 업무 흐름 | 화면 이동 증가, 컨텍스트 단절, 실수 유발 |
| 3 | **확장성 부족** — 신규 기능 추가 시 전체 수정·재배포 | 고객 대응 속도 저하, 유지보수 비용 상승 |

---

### 해결 방향 (→ [섹션 4](#4-핵심-해결-방향) 상세)

| 방향 | 내용 |
|------|------|
| **Web 기반** | 브라우저만으로 접근, OS 독립성 확보 |
| **Workflow UI** | 접수→처방→조제→검토→수납→청구 단계별 Stepper |
| **Plugin 구조** | 핵심 기능과 확장 기능 분리, 플러그인 ON/OFF 관리 |

---

### 핵심 기능 요약 (→ [섹션 5](#5-기능-명세) 상세)
...
[조제] 처방 항목 체크리스트 확인
  ↓
[검토] 처방 요약 확인, DUR·복약지도 Plugin 실행
  ↓
[수납] 본인부담금 자동 계산 (약제비 × 20~30%), 결제 방법 선택
  ↓
[청구] 건강보험 청구 데이터 생성, 업무 완료
```

**구현된 Plugin:**
- `DUR` — 약물 상호작용·금기 여부 검사
- `medication-guide` — 처방 약품별 복용 안내문 자동 생성

---

### 기술 스택 요약 (→ [섹션 6](#6-기술-스택) 상세)
...
| 배포 | Vercel (Frontend CDN + Backend Serverless Function) |
| CI/CD | GitHub Actions (Lint → Test → Build → Deploy → Smoke Test) |

---

### 아키텍처 한 눈에 보기 (→ [섹션 7](#7-시스템-아키텍처) 상세)

```
Frontend (React)          Backend (Express)
  Zustand (상태)   ←API→   routes → services → domain
  Axios (호출)              plugins (확장)
                            Prisma ↓
                          PostgreSQL (Neon)
```

**레이어 원칙:** `domain/`은 외부 의존 없는 순수 TypeScript. `routes/`는 `services/`만 호출.
...
| 항목 | 결과 |
|------|------|
| Workflow 6단계 | ✅ 전 단계 구현 및 DB 연동 동작 |
| 테스트 | ✅ 86개 (Frontend 20 + Backend Unit 22 + 통합 44), 전체 PASS |
| 커버리지 | ✅ Statements 89.38% / Domain 100% |
| CI/CD | ✅ 5단계 파이프라인 (Smoke Test 포함) |
| 배포 | ✅ https://pharm-weave-frontend.vercel.app |
...
| 섹션 | 내용 |
|------|------|
| [1. 프로젝트 개요](#1-프로젝트-개요) | 목표 및 핵심 한 문장 |
| [2. 문제 정의](#2-문제-정의) | 현 PMS의 한계, 근거 자료 5건 |
| [3. 대상 사용자](#3-대상-사용자) | 약사·실무자·관리자 역할 정의 |
| [4. 핵심 해결 방향](#4-핵심-해결-방향) | Web·Workflow·Plugin 3가지 방향 |
| [5. 기능 명세](#5-기능-명세) | 6단계 Workflow API·UI 상세 명세 |
| [6. 기술 스택](#6-기술-스택) | 선택 이유 및 버전 명세 |
| [7. 시스템 아키텍처](#7-시스템-아키텍처) | 레이어 구조·데이터 흐름 |
| [8. 데이터 모델](#8-데이터-모델) | Prisma 스키마·ERD |
| [9. API 설계](#9-api-설계) | 전체 엔드포인트 명세 |
| [12. 테스트 전략](#12-테스트-전략) | 단위·통합·CI 전략 |
| [13. 성공 지표](#13-성공-지표) | 데모 체크리스트·성능 목표·평가 기준 |
...
> 프로덕션 운영 전환 시 추적할 핵심 지표. 해커톤 단계에서는 데모 기반 기준값을 설정하고 배포 후 실측으로 검증한다.

#### 13.3.1 업무 효율 지표

| KPI | 목표값 | 측정 방법 | 기준 근거 |
|-----|--------|-----------|-----------|
| 전체 Workflow 완주율 (접수→청구 오류 없이) | ≥ 95% | 방문 완료 건수 / 방문 생성 건수 | 기존 WinForms PMS 수작업 오류율 ~8% 감안 |
| 단계별 전환 소요 시간 (사용자 작업 기준) | ≤ 3분 / 단계 | 단계 생성~완료 타임스탬프 delta | 숙련 약사 1회 처방 처리 평균 5분 목표 절반 |
| 약품 검색 선택까지 클릭 수 | ≤ 3회 | UX 관찰 | 자동완성 드롭다운 도입 근거 |
| 환자 등록 오류(필드 재입력) 발생률 | ≤ 5% | 폼 submit 실패 / 전체 submit | 필드 레벨 유효성 검사 도입 근거 |

#### 13.3.2 기술 품질 지표

| KPI | 목표값 | 현재 달성값 | 측정 도구 |
|-----|--------|-------------|-----------|
| 테스트 커버리지 (Statements) | ≥ 90% | **98%+** | Jest --coverage |
| Domain Layer 커버리지 | 100% | **100%** | Jest --coverage |
| ESLint 경고 수 | 0 | **0** | ESLint max-warnings 0 |
| CI 빌드 성공률 (최근 10회) | ≥ 90% | GitHub Actions 이력 | GitHub Actions |
| 프로덕션 Smoke Test SLO (p95) | < 3,000ms | CI 7단계 자동 측정 | curl --write-out |

#### 13.3.3 가용성 지표

| KPI | 목표값 | 측정 방법 |
|-----|--------|-----------|
| 배포 후 자동 롤백 발동 조건 | 7단계 Smoke Test 실패 즉시 | CI 8단계 Auto Rollback |
| 롤백 완료까지 소요 시간 | ≤ 2분 | vercel rollback CLI 실측 |
| 동시 사용자 요청 처리 성공률 | ≥ 80% (10 concurrent) | CI 7단계 동시성 테스트 |

### 13.4 평가 기준 매핑 (총 100점 + 보너스 10점)
